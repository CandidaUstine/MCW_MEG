...
OrgFile = open ('run02_oddball_left.eve', 'rb')
OutputFile = open('run02_oddball_left_det_mod' + '.eve', 'wb')
...
def modify_file(file_name_in):
        ...
        seq_num = -1
	insequence = False
        with open('run02_oddball_left_det_mod.eve', 'w') as output_file_obj:
                for line in lines:
                        magic_part = line.strip().split()
...
OrgFile.close()
OutputFile.close()
modify_file('run02_oddball_left_det_mod.eve')
