...
OutputFile = open('run03_dual_oddball_left' + '_mod' + '.eve', 'wb')
...
def simple_modify_file(file_name_in):
    ...
    seq_num = -1
    insequence = False
    with open('run03_dual_oddball_left_mod.eve', 'w') as output_file_obj:
        for line in lines:
            magic_part = line.strip().split()
...
OrgFile.close()
OutputFile.close()  
simple_modify_file('run03_dual_oddball_left_mod.eve')
