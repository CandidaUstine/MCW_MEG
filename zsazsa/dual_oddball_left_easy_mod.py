...
OrgFile = open ('run03_dual_oddball_left_raw.eve', 'rb')
OutputFile = open('run03_dual_oddball_left_raw' + '_mod' + '.eve', 'wb')
...
def simple_modify_file(file_name_in):
    ...
    seq_num = -1
    insequence = False
    with open('run03_dual_oddball_left_raw_mod.eve', 'w') as output_file_obj:
        for line in lines:
            magic_part = line.strip().split()
...
OrgFile.close()
OutputFile.close()  
simple_modify_file('run03_dual_oddball_left_raw_mod.eve')
